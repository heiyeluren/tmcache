...
int main(){
	...
	//create
	struct tm_queue_t *ql;
	ql = tm_qcreate();
	printf("Create queue list.\n");

	//is empty
	printf("Queue List empty status: %d\n", tm_qempty(ql));
	...
	char *key1 = "key2";
	unsigned data_len1 = 100;
	printf("Entry node in queue. key:%s, datalen:%d, result:%d\n", key, data_len, tm_qentry(ql, key, data_len));
	printf("Entry node in queue. key:%s, datalen:%d, result:%d\n", key1, data_len1, tm_qentry(ql, key1, data_len1));
	...
	struct tm_queue_node_t *node;
	node = tm_qremove(ql);
	printf("Remove node from queue. key:%s, datalen:%d\n", node->key, node->data_len);

	//visit
	tm_qvisit(ql);

	//remove
	struct tm_queue_node_t *node1;
	node1 = tm_qremove(ql);
	printf("Remove node from queue. key:%s, datalen:%d\n", node1->key, node1->data_len);

	//visit
	tm_qvisit(ql);

	//destroy
	printf("Destroy queue result: %d\n", tm_qdestroy(ql));
